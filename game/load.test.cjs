...
test('300 live players: simultaneous buzz, authorization, reconnect and restart', { timeout: 120000 }, async t => {
  ...
  await cmd('correct');
  const score = app.game.state.scores[winner.team];
  const replay = await emit(host, 'command', { action: 'correct', version: app.game.state.version });
  assert.ok(replay.error); assert.equal(app.game.state.scores[winner.team], score);
  ...
  assert.equal(app.game.state.scores[winner.team], 100);
  assert.equal(Object.keys(app.game.state.players).length, 300);
